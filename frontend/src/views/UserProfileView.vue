...
const toggleConnect = async () => {
  if (connecting.value || isMe.value) return
  connecting.value = true
  try {
    const data = await apiFetch(`/api/social/connect/${route.params.id}`, { method: 'POST' })
    if (profile.value) {
      profile.value.isFollowing = data.following
      if (data.following) profile.value.followerCount++
      else profile.value.followerCount = Math.max(0, (profile.value.followerCount || 1) - 1)
    }
  } catch { /* ignore */ }
...
<template>
  <div class="max-w-2xl mx-auto py-4 flex flex-col gap-6">
    ...
    <template v-else-if="profile">
      ...
      <div class="p-6 rounded-2xl border border-white/8 flex flex-col sm:flex-row items-center sm:items-start gap-5"
        ...
        <div class="flex-1 text-center sm:text-left">
          <div class="flex items-center justify-center sm:justify-start gap-2 flex-wrap">
            ...
            <span v-if="profile.user.plan === 'premium'"
          ...
          <p v-if="profile.user.bio" class="text-white/45 text-sm mt-1 italic">{{ profile.user.bio }}</p>
          <p v-if="memberSince" class="text-white/25 text-xs mt-1">{{ t.memberSince }} {{ memberSince }}</p>

          <!-- Social counts -->
          <div class="flex items-center justify-center sm:justify-start gap-4 mt-3">
            <div class="text-center">
              <p class="text-white font-bold text-lg leading-none">{{ profile.followerCount }}</p>
              <p class="text-white/35 text-xs mt-1">{{ t.followers }}</p>
            </div>
            <div class="w-px h-8 bg-white/10"></div>
            <div class="text-center">
              <p class="text-white font-bold text-lg leading-none">{{ profile.followingCount }}</p>
              <p class="text-white/35 text-xs mt-1">{{ t.following }}</p>
            </div>
          </div>
        </div>
        ...
        <div v-if="!isMe" class="flex-shrink-0">
          <button
            ...
            :class="isFollowing
              ? 'border border-white/15 text-white/50 hover:border-red-500/40 hover:text-red-400 hover:bg-red-500/8'
              : 'text-white'"
            :style="!isFollowing ? 'background: linear-gradient(135deg, #5b21b6, #7c3aed);' : ''"
            :disabled="connecting"
            @click="toggleConnect"
          >
            <i v-if="connecting" class="fa-solid fa-circle-notch fa-spin text-xs"></i>
            <i v-else-if="isFollowing" class="fa-solid fa-user-check text-xs"></i>
            <i v-else class="fa-solid fa-film text-xs"></i>
            {{ isFollowing ? t.inCredits : t.joinCredits }}
          </button>
        </div>
      </div>

      <!-- Watcher title (if available — from future extension) -->
      <!-- We don't fetch this for public profiles yet, but placeholder left here -->

      <!-- Liked -->
      <section v-if="profile.liked !== null" class="flex flex-col gap-4">
        ...
        <div v-if="profile.liked?.length" class="grid grid-cols-3 sm:grid-cols-5 gap-3">
          <div
          ...
          >
            <img v-if="item.poster" :src="`${IMG_BASE}${item.poster.replace(/^.*\/p\/w\d+/,'')}`" :alt="item.title" class="w-full aspect-[2/3] object-cover" />
            <div v-else class="w-full aspect-[2/3] bg-white/5 flex items-center justify-center">
              <i class="fa-solid fa-film text-white/20"></i>
            </div>
          </div>
        </div>
        <p v-else class="text-white/30 text-sm italic">{{ t.empty }}</p>
      </section>
      <div v-else-if="profile.liked === null" class="p-4 rounded-xl bg-white/3 border border-white/7 text-white/30 text-sm">
        <i class="fa-solid fa-lock mr-2 text-xs"></i>{{ t.libraryPrivate }}
      </div>

      <!-- Watchlist -->
      <section v-if="profile.watchlist !== null" class="flex flex-col gap-4">
        <h2 class="text-xs uppercase tracking-widest text-white/40 font-medium flex items-center gap-2">
          <i class="fa-solid fa-bookmark text-blue-400/60 text-xs"></i>
          {{ t.myWatchlist }} ({{ profile.watchlist?.length || 0 }})
        </h2>
        <div v-if="profile.watchlist?.length" class="grid grid-cols-3 sm:grid-cols-5 gap-3">
          <div
      ...
      <div v-else-if="profile.watchlist === null" class="p-4 rounded-xl bg-white/3 border border-white/7 text-white/30 text-sm">
        <i class="fa-solid fa-lock mr-2 text-xs"></i>{{ t.libraryPrivate }}
      </div>

      <!-- Watched -->
      <section v-if="profile.watched !== null" class="flex flex-col gap-4">
        <h2 class="text-xs uppercase tracking-widest text-white/40 font-medium flex items-center gap-2">
          <i class="fa-solid fa-check-circle text-green-400/60 text-xs"></i>
          {{ t.myWatched }} ({{ profile.watched?.length || 0 }})
        </h2>
        <div v-if="profile.watched?.length" class="grid grid-cols-3 sm:grid-cols-5 gap-3">
          <div
      ...
      <div v-else-if="profile.watched === null" class="p-4 rounded-xl bg-white/3 border border-white/7 text-white/30 text-sm">
        <i class="fa-solid fa-lock mr-2 text-xs"></i>{{ t.libraryPrivate }}
      </div>
